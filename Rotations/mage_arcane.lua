...
function mage_arcane()
	...
	spellTable[1] = {
		...
		{ "mirror image", jps.UseCDs },
		{ {"macro","/use Mana Gem"}, mana < 0.8 and GetItemCount("Mana Gem", 0, 1) > 0 and not jps.buff("alter time"), player }, 
		...
		{ jps.DPSRacial, (jps.UseCDs and not jps.buff("alter time") ) and ( jps.buff("arcane power") or jps.TimeToDie("target") < 15) },
		-- On-use Trinkets.
